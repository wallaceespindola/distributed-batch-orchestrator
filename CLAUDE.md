...
./scripts/start-local.sh [--build] [--with-frontend]   # 6 instances on 8080-8085 (+frontend :3000)
./scripts/stop-local.sh                 # stop everything (ps1 variants exist for Windows)
make help                               # all Makefile targets
docker compose up --build               # kubernetes-mode topology locally (Kafka + H2 server + 6 apps)
...
Distributed batch app: Java 21, Spring Boot 3.4, Spring Batch, H2. Six **identical** instances (a Master is also a Worker — it processes a partition like any peer); the first to receive `POST /api/batch/start` wins a ShedLock JDBC lock and is Master for that run (lock released by the job-completion listener → role rotates). Concurrent starts → 409.
...
- **`kubernetes`:** Kafka remote partitioning (`config/KafkaBatchConfig`, `@EnableBatchIntegration`): manager step sends `StepExecutionRequest`s to topic `batch-partition-requests` (Java serialization — `batch/kafka/JavaSerializer`), 6 pods consume in group `batch-workers`, manager polls the shared job repository (H2 TCP server pod, `DB_URL` env) for completion. Every pod carries both manager and worker beans.
...
- Startup data seeding races across 6 instances — guarded by ShedLock (`data-init`) + empty check in `DataGeneratorService`.
...
- All 6 instances must stay byte-identical: never add instance-specific config, images or code paths.
- **Cluster size 6 is spread across many files** — changing it means updating all of: `KafkaBatchConfig.GRID_SIZE` (also the topic partition count), `k8s/app.yaml` replicas, `docker-compose.yml` `app-N` services, `scripts/start-local.{sh,ps1}` port arrays, `app.peers` in `application.yml`, `.env.example`, `frontend/js/app.js` `CLUSTER_PORTS`, README/specs.
